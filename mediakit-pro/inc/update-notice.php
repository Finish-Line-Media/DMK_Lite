...
function mkp_display_update_notice() {
    ...
    <div class="notice notice-info is-dismissible mkp-update-notice" data-version="<?php echo esc_attr( $remote_version ); ?>">
        <p>
            <strong><?php esc_html_e( 'MediaKit Pro Theme Update Available!', 'mediakit-pro' ); ?></strong>
        </p>
        <p>
            <?php
            printf(
                esc_html__( 'A new version (%1$s) of MediaKit Pro is available. You are currently using version %2$s.', 'mediakit-pro' ),
        ...
        <p>
            <a href="<?php echo esc_url( $download_url ); ?>" class="button button-primary" target="_blank">
                <?php esc_html_e( 'Download Update', 'mediakit-pro' ); ?>
            </a>
            
            <?php if ( $changelog_url ) : ?>
                <a href="<?php echo esc_url( $changelog_url ); ?>" class="button" target="_blank">
                    <?php esc_html_e( 'View Changelog', 'mediakit-pro' ); ?>
                </a>
            <?php endif; ?>
            
            <a href="<?php echo esc_url( admin_url( 'themes.php' ) ); ?>" class="button">
                <?php esc_html_e( 'Go to Themes', 'mediakit-pro' ); ?>
            </a>
        </p>
        
        <p>
            <small>
                <?php esc_html_e( 'To update: 1) Download the new version, 2) Go to Themes, 3) Delete the current theme, 4) Upload the new mediakit-pro.zip file.', 'mediakit-pro' ); ?>
            </small>
        </p>
    </div>
...
function mkp_add_theme_update_info() {
    ...
    if ( $current_theme->get( 'Name' ) !== 'MediaKit Pro' ) {
        return;
    }
    ...
    if ( version_compare( $current_version, $remote_version, '<' ) ) {
        add_action( 'admin_footer', function() use ( $remote_version ) {
            ...
            <script>
            jQuery(document).ready(function($) {
                var themeCard = $('.theme[data-slug="mediakit-pro"]');
                if (themeCard.length) {
                    themeCard.addClass('update');
                    themeCard.find('.theme-name').append('<span class="update-message"><?php echo esc_js( sprintf( __( 'Update Available: %s', 'mediakit-pro' ), $remote_version ) ); ?></span>');
                }
            });
            </script>
            <style>
            .theme[data-slug="mediakit-pro"] .update-message {
                display: block;
                font-size: 12px;
                color: #d63638;
                margin-top: 5px;
