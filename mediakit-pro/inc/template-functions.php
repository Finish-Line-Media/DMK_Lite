...
function mkp_excerpt_more_link( $more ) {
    if ( ! is_admin() ) {
        global $post;
        return '... <a class="mkp-read-more" href="' . esc_url( get_permalink( $post->ID ) ) . '">' . __( 'Read More', 'mediakit-pro' ) . '</a>';
    }
    return $more;
}
...
function mkp_search_form( $form ) {
    $form = '<form role="search" method="get" class="mkp-search-form" action="' . esc_url( home_url( '/' ) ) . '">
        <label>
            <span class="screen-reader-text">' . esc_html__( 'Search for:', 'mediakit-pro' ) . '</span>
            <input type="search" class="mkp-search-form__field" placeholder="' . esc_attr__( 'Search...', 'mediakit-pro' ) . '" value="' . get_search_query() . '" name="s" />
        </label>
        <button type="submit" class="mkp-search-form__submit">
            <span class="screen-reader-text">' . esc_html__( 'Search', 'mediakit-pro' ) . '</span>
...
function mkp_comment_form_fields( $fields ) {
    ...
    $fields['author'] = '<p class="comment-form-author mkp-form-group">
        <label for="author">' . __( 'Name', 'mediakit-pro' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label>
    ...
    $fields['email'] = '<p class="comment-form-email mkp-form-group">
        <label for="email">' . __( 'Email', 'mediakit-pro' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label>
    ...
    $fields['url'] = '<p class="comment-form-url mkp-form-group">
        <label for="url">' . __( 'Website', 'mediakit-pro' ) . '</label>
...
function mkp_star_rating( $rating, $max = 5 ) {
    $output = '<div class="mkp-star-rating" aria-label="' . sprintf( esc_attr__( 'Rating: %s out of %s stars', 'mediakit-pro' ), $rating, $max ) . '">';
...
function mkp_get_file_size( $file_url ) {
    ...
    return __( 'Unknown', 'mediakit-pro' );
}
...
function mkp_share_buttons( $post_id = null ) {
    ...
    <div class="mkp-share-buttons">
        <span class="mkp-share-buttons__label"><?php esc_html_e( 'Share:', 'mediakit-pro' ); ?></span>
...
function mkp_password_form() {
    ...
    $output = '<form class="mkp-password-form" action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" method="post">
        <p>' . __( 'This content is password protected. To view it please enter your password below:', 'mediakit-pro' ) . '</p>
        <div class="mkp-form-group">
            <label for="' . $label . '">' . __( 'Password:', 'mediakit-pro' ) . '</label>
            <input class="mkp-form-control" name="post_password" id="' . $label . '" type="password" size="20" />
        </div>
        <button type="submit" class="mkp-btn mkp-btn--primary">' . esc_attr__( 'Enter', 'mediakit-pro' ) . '</button>
    </form>';
...
function mkp_get_front_page_nav_items() {
    ...
    if ( get_theme_mod( 'mkp_enable_section_speaker_topics', true ) && $has_topics ) {
        $nav_items[] = array(
            'id'    => 'speaking',
            'label' => __( 'Speaking', 'mediakit-pro' ),
            'url'   => '#speaking',
        );
    }
    ...
    if ( get_theme_mod( 'mkp_enable_section_corporations', true ) && $has_corps ) {
        $nav_items[] = array(
            'id'    => 'corporations',
            'label' => __( 'Corporations', 'mediakit-pro' ),
            'url'   => '#corporations',
        );
    }
    ...
    if ( get_theme_mod( 'mkp_enable_section_media_questions', true ) && $has_questions ) {
        $nav_items[] = array(
            'id'    => 'media-questions',
            'label' => __( 'Interview Q&A', 'mediakit-pro' ),
            'url'   => '#media-questions',
        );
    }
    ...
    if ( get_theme_mod( 'mkp_enable_section_investor', true ) && 
        ...
        $nav_items[] = array(
            'id'    => 'investor',
            'label' => __( 'Investor', 'mediakit-pro' ),
            'url'   => '#investor',
        );
    }
