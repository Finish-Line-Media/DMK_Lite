...
function mkp_prevent_theme_switch_changeset() {
    ...
    if ( isset( $_GET['page'] ) && $_GET['page'] === 'mediakit-lite' && function_exists( 'mkp_debug_log' ) ) {
        mkp_debug_log( 'Coming from MediaKit admin page' );
    }
    
    // If this is a theme preview for our own theme, that's OK
    if ( isset( $_GET['theme'] ) && $_GET['theme'] === 'mediakit-lite' && get_option( 'stylesheet' ) === 'mediakit-lite' && function_exists( 'mkp_debug_log' ) ) {
...
function mkp_fix_customizer_changeset( $data, $filter_context ) {
    ...
    if ( isset( $data['old_sidebars_widgets_data'] ) && function_exists( 'mkp_debug_log' ) ) {
